// ==============================================================================
// 🪐 PROJEKT "Q-O" // CONTENT.JS - PORTRÄT HUD HOVER VISIBILITY & PLUMP BLOB
// ==============================================================================
// AXIOM XXXIV: DAS GESETZ DES STATISCHEN ANKERS & PRALLER VEKTOR-RADIUS
...
// 3. SÄULE III: AXIOM XII - DEZENTRALES PROXIMITY-GEFÄLLE (Score < 0.5 -> Dichte-basiertes Amethyst-Skelett; < 0.25 -> 2.4px Kollaps)
// + KINÄSTHETISCHES MAUS-GETRIEBE MIT REAKTIVER HOVER-LICHT-PROJEKTION (-12px, 12px)
// ==============================================================================
...
(function () {
  ...
  // Dezentraler Teer-Partikel-Pool (Striktes 40-Orb Static-RAM-Pool-Verfahren)
  const MAX_TAR_PARTICLES = 40;
  const tarParticles = [];
  let tarParticleElements = [];
  ...
  // ============================================================================
  // 2. DOM-INJEKTION: STATISCHE MATRIX-HAUT & IPHONE-HOCHFORMAT (140x220px)
  // ============================================================================
  function injectMetabolicWidget() {
    ...
    // Gespeicherte Koordinaten aus dem RAM / Storage abrufen
    try {
      const savedCoords = localStorage.getItem('qo_widget_coords');
      if (savedCoords) {
        const parsed = JSON.parse(savedCoords);
        if (typeof parsed.x === 'number' && typeof parsed.y === 'number') {
          const maxLeft = Math.max(10, window.innerWidth - 150);
          const maxTop = Math.max(10, window.innerHeight - 230);
          const boundedX = Math.min(Math.max(10, parsed.x), maxLeft);
          const boundedY = Math.min(Math.max(10, parsed.y), maxTop);
    ...
    container.innerHTML = `
      <!-- 📐 DAS IPHONE-COCKPIT-PANEL (Materialisiert sich rein bei Hover & Drag als reaktiver Licht-Hintergrund) -->
      <div id="q-o-cockpit-panel">
        <!-- ✨ MODUL 2: DIAGONALE LICHT-LINSEN (1px Vektor-Linien, absolut kein Weichzeichner) -->
        <div class="qo-diagonal-corner qo-corner-tl"></div>
        <div class="qo-diagonal-corner qo-corner-br"></div>

        <!-- 🌌 MODUL 5: DAS UNTERE 61.8% FACH (Steriles Vektor-Reservoir mit Deep Blur 24px) -->
        <div id="qo-lower-matrix-compartment"></div>
      </div>

      <!-- 🧬 MODUL 3: DAS OBERE 38.2% FACH (Goldene Koinzidenz-Verriegelung für das Organell) -->
      <div id="biomorphic-blob-wrapper">
        <svg id="biomorphic-blob" viewBox="0 0 100 100">
          <defs>
          ...
          <g id="qo-background-celestial-rings" opacity="0.22">
            <circle cx="50" cy="50" r="38" fill="none" stroke="rgba(0, 240, 255, 0.22)" stroke-width="0.35" stroke-dasharray="2 4" />
            <circle cx="50" cy="50" r="28" fill="none" stroke="rgba(168, 85, 247, 0.22)" stroke-width="0.35" />
            <circle cx="50" cy="50" r="18" fill="none" stroke="rgba(0, 240, 255, 0.15)" stroke-width="0.3" stroke-dasharray="1 2" />
          </g>
          ...
          <path id="qo-inner-refract-1" fill="none" stroke="url(#qo-filament-refract-grad)" stroke-width="0.8" stroke-linecap="round" opacity="0.7" />
          <path id="qo-inner-refract-2" fill="none" stroke="#00f0ff" stroke-width="0.7" stroke-linecap="round" opacity="0.6" />

          <!-- ✨ SPITZLICHTER & KINETISCHE SATELLITEN-ORBS -->
          <circle id="qo-crest-light-1" cx="38" cy="37" r="1.4" fill="#ffffff" opacity="0.95" />
          <circle id="qo-crest-light-2" cx="62" cy="38" r="1.3" fill="#ffffff" opacity="0.9" />
          <circle id="qo-sat-orb-cw" cx="30" cy="40" r="1.0" fill="#00f0ff" opacity="0.75" />
          <circle id="qo-sat-orb-ccw" cx="70" cy="60" r="1.0" fill="#e879f9" opacity="0.7" />
        </svg>
      </div>
    `;
    ...
    // Pre-populate tarSwarmGroup with 40 static circle elements (Zero DOM creations in 60-FPS loop)
    const swarmGroup = document.getElementById('qo-tar-bubbles-swarm');
    if (swarmGroup) {
      tarParticleElements = [];
      let circlesSvg = '';
      for (let i = 0; i < MAX_TAR_PARTICLES; i++) {
        circlesSvg += `<circle id="qo-tar-orb-${i}" cx="50" cy="50" r="1.1" fill="url(#qo-tar-particle-grad)" stroke="#010005" stroke-width="0.35" opacity="0" display="none" />`;
      }
      swarmGroup.innerHTML = circlesSvg;
      for (let i = 0; i < MAX_TAR_PARTICLES; i++) {
        const el = document.getElementById(`qo-tar-orb-${i}`);
        if (el) tarParticleElements.push(el);
      }
    }

    initKineticDragEngine();
  }

  // ============================================================================
  // 3. KINÄSTHETISCHES MAUS-GETRIEBE (DRAG & DROP ENGINE MIT ISOLIERUNG)
  // ============================================================================
  function initKineticDragEngine() {
    const container = document.getElementById('q-o-widget-container');
    const blobWrapper = document.getElementById('biomorphic-blob-wrapper');
    const panel = document.getElementById('q-o-cockpit-panel');
    if (!container || !blobWrapper) return;
    ...
    function onMouseDown(e) {
      ...
      if (blobWrapper) blobWrapper.classList.add('is-dragging');
    ...
    function onMouseMove(e) {
      ...
      if (!dragThresholdPassed && (Math.abs(deltaX) > 3 || Math.abs(deltaY) > 3)) {
        dragThresholdPassed = true;
      }

      let newX = initialContainerX + deltaX;
      let newY = initialContainerY + deltaY;

      // Viewport-Kollisionsschutz
    ...
    function onMouseUp(e) {
      ...
      if (blobWrapper) blobWrapper.classList.remove('is-dragging');
      ...
      // Speicherung der finalen Koordinaten
      ...
      // Reiner Klick ohne signifikanten Drag: Labor & Biopsie
      if (!dragThresholdPassed) {
        sealBiopsyAndOpenLab();
      }
    }

    blobWrapper.addEventListener('mousedown', onMouseDown);
  }

  // ============================================================================
  // 4. MATHEMATISCHE VEKTOR-INTERPOLATION
  // ============================================================================
  ...
  // ============================================================================
  // 5. LIVE 60-FPS VEKTOR-RENDERSCHLEIFE (PRALLE PROPORTION: RADIUS 29.5 * hostScale)
  // ============================================================================
  function renderFluidVortex(timestamp) {
    ...
    if (!isKryoSleep) {
      ...
      if (currentLqScore >= 0.5) {
        ...
        targetStroke = awakeningProgress * 1.6; // Stufenlos 0.0 -> 1.6px
      } else {
        targetStroke = 2.4; // Metabolischer Kollaps -> 2.4px
      }
      ...
      if (currentLqScore < 0.25) {
        hostScale = 0.82 + (currentLqScore / 0.25) * 0.15;
        const tremorAmp = (0.25 - currentLqScore) * 2.0;
        centerJitterX = (Math.random() - 0.5) * tremorAmp;
        centerJitterY = (Math.random() - 0.5) * tremorAmp;
      }

      // Koinzidenz-Verriegelung: Mittelpunkt starr bei (50, 50) im oberen 38.2%-Fach
      const cx = 50 + centerJitterX;
      const cy = 50 + centerJitterY;

      // 🪐 AXIOM XXXIV: PRALLER WIDGET-RADIUS (29.5 * hostScale - kein Anecken an den Flanken)
      const baseOuterRadius = 29.5 * hostScale;
      ...
      const amp1 = (3.8 + Math.sin(phaseHarmonicB) * 0.9) * (1.0 - Math.min(0.4, viscoelasticStrain * 0.3));
      const amp2 = (2.2 + Math.cos(phaseHarmonicA) * 0.6) * (1.0 - Math.min(0.4, viscoelasticStrain * 0.3));
      ...
      for (let i = 0; i < numOuterPoints; i++) {
        ...
        const strainDip = viscoelasticStrain > 0.02
          ? Math.abs(Math.sin(4 * theta + phaseShadowCCW)) * (viscoelasticStrain * 2.8)
          : 0;
      ...
      // =======================================================================
      // E. SÄULE II: UNSICHTBARE RAM-KOORDINATEN (ghostCapillaryNodes // 8 BUCHTEN)
      // =======================================================================
      ...
      for (let b = 0; b < numBays; b++) {
        ...
        // Dezentral an der äußeren Peripherie entlanggekrümmt
        const pEntry = {
          x: cx + Math.cos(bayTheta + phaseShadowCCW * 0.22) * (baseOuterRadius - 2.5),
          y: cy + Math.sin(bayTheta + phaseShadowCCW * 0.22) * (baseOuterRadius - 2.5)
        };
        ...
        const pEnd = {
          x: cx + Math.cos(bayTheta + 0.65 + phaseShadowCCW * 0.18) * (baseOuterRadius * 0.52),
          y: cy + Math.sin(bayTheta + 0.65 + phaseShadowCCW * 0.18) * (baseOuterRadius * 0.52)
        };
      ...
      // =======================================================================
      // F. SÄULE II: RADIALER INTERSECTION-SPAWN DER TEER-ORBS (STRIKT #010005)
      // =======================================================================
      // 1. Logik-Check & Sclerosis-Zustand proportional zum Mond-Regler (intensity: 0.0 - 1.0)
      const intensity = Math.max(0.0, Math.min(1.0, currentSensitivity));
      const sclerosisThreshold = intensity * 0.45;
      const isSclerosis = intensity > 0.0 && currentLqScore < sclerosisThreshold;

      const widgetContainer = document.getElementById('q-o-widget-container');
      if (widgetContainer) {
        widgetContainer.setAttribute('data-sclerosis', isSclerosis ? 'active' : 'inactive');
      }

      // 2. 32-Schritte Radialer Intersection-Spawn (Die Geburtsstätten)
      const spawnDelta = 0.5 + (intensity * 1.5);

      if (isSclerosis && !isKryoSleep) {
        for (let k = 0; k < 32; k++) {
          const theta = (k / 32) * Math.PI * 2;

          // Modul Alpha (Helix CW)
          const waveAlpha =
            Math.sin(5 * theta + phaseHelixCW) * amp1 +
            Math.cos(3 * theta - phaseHarmonicB) * amp2;
          const radiusAlpha = Math.max(8, baseOuterRadius + waveAlpha);

          // Modul Beta (Shadow CCW)
          const waveBeta =
            Math.cos(4 * theta + phaseShadowCCW) * (amp1 * 0.8) +
            Math.sin(2 * theta - phaseHarmonicA) * (amp2 * 0.8);
          const radiusBeta = Math.max(8, baseOuterRadius * 0.88 + waveBeta);

          if (Math.abs(radiusAlpha - radiusBeta) < spawnDelta) {
            // Kontrollierte Spawn-Chance pro Frame (Verhindert Überfüllung in 1 Frame)
            if (Math.random() < 0.12 * (0.4 + intensity * 0.6)) {
              const freeOrb = tarParticles.find((p) => !p.active);
              if (freeOrb) {
                freeOrb.active = true;
                const birthR = (radiusAlpha + radiusBeta) * 0.5;
                freeOrb.x = cx + Math.cos(theta) * birthR;
                freeOrb.y = cy + Math.sin(theta) * birthR;
                freeOrb.birthX = freeOrb.x;
                freeOrb.birthY = freeOrb.y;
                freeOrb.angle = theta + (Math.random() - 0.5) * 0.35;
                freeOrb.radialSpeed = 0.14 + Math.random() * 0.24;
                freeOrb.baseRadius = 0.85 + Math.random() * 0.95;
                freeOrb.pulsePhase = Math.random() * Math.PI * 2;
                freeOrb.opacity = 1.0;
                freeOrb.life = 0.0;
                freeOrb.maxLife = 1.8 + Math.random() * 2.2;
                freeOrb.bayIndex = Math.floor((theta / (Math.PI * 2)) * numBays) % numBays;
              }
            }
          }
        }
      ...
      // 3. Kinetischer Drift, Recycling & Zero-DOM-Allocation State
      for (let i = 0; i < MAX_TAR_PARTICLES; i++) {
        const p = tarParticles[i];
        const el = tarParticleElements[i];

        if (p.active && !isKryoSleep) {
          p.life += dt;
          p.pulsePhase += 2.8 * dt;

          // Trägheitsgedämpfter radialer Drift nach außen
          const driftSpeed = p.radialSpeed * (1.0 + viscoelasticStrain * 0.75) * 18.0 * dt;
          ...
          // Recycling-Trigger: Lebensdauer-Limit oder Verlassen des 80px-Backdrop-Radius (SVG-Radius ca. 44)
          const isExiting = p.life >= p.maxLife || dist >= 44.0 || !isSclerosis;

          if (isExiting) {
            p.opacity -= 3.5 * dt;
            if (p.opacity <= 0.0) {
              p.opacity = 0.0;
              p.active = false; // Zurück in den statischen Pool
            }
          } else {
            const lifeProgress = p.life / p.maxLife;
            p.opacity = Math.max(0.0, Math.min(1.0, 1.0 - lifeProgress * 0.35));
            bayParticleCounts[p.bayIndex]++;
          }

          if (el) {
            if (p.active && p.opacity > 0.01) {
              const pulse = 0.88 + Math.sin(p.pulsePhase) * 0.22;
              const curRadius = p.baseRadius * pulse * (1.0 + viscoelasticStrain * 0.4);
              el.setAttribute('cx', p.x.toFixed(1));
              el.setAttribute('cy', p.y.toFixed(1));
              el.setAttribute('r', curRadius.toFixed(1));
            ...
            if (lineEl && curve) {
              ...
              if (density > 0 || isKollaps) {
                const localStroke = isKollaps 
                  ? 2.4 
                  : Math.min(1.6, globalSkeletonStroke * (0.6 + Math.min(1.0, density * 0.25)));
      ...
      if (refract1) {
        ...
        const x1 = cx + Math.cos(a1) * (21 * hostScale);
        const y1 = cy + Math.sin(a1) * (21 * hostScale);
        const x2 = cx + Math.cos(a1 + 1.1) * (13 * hostScale);
        const y2 = cy + Math.sin(a1 + 1.1) * (13 * hostScale);
        const x3 = cx + Math.cos(a1 + 2.0) * 4;
        const y3 = cy + Math.sin(a1 + 2.0) * 4;
      ...
      if (refract2) {
        ...
        const x1 = cx + Math.cos(a2) * (18 * hostScale);
        const y1 = cy + Math.sin(a2) * (18 * hostScale);
        const x2 = cx + Math.cos(a2 + 1.0) * (11 * hostScale);
        const y2 = cy + Math.sin(a2 + 1.0) * (11 * hostScale);
        const x3 = cx + Math.cos(a2 + 1.9) * 4;
        const y3 = cy + Math.sin(a2 + 1.9) * 4;
      ...
      if (satCW) {
        ...
        satCW.setAttribute('cx', (50 + Math.cos(sA) * 37).toFixed(1));
        satCW.setAttribute('cy', (50 + Math.sin(sA) * 34).toFixed(1));
      }
      if (satCCW) {
        const sB = phaseShadowCCW * 0.5 + 2.2;
        satCCW.setAttribute('cx', (50 + Math.cos(sB) * 35).toFixed(1));
        satCCW.setAttribute('cy', (50 + Math.sin(sB) * 37).toFixed(1));
      }
    }
  ...
  // ============================================================================
  // 6. DOM-TEXTEXTRAKTION & HASH-PRÜFUNG (4s METABOLISMUS-PULS)
  // ============================================================================
  ...
  // ============================================================================
  // 7. ANALYSE-ABFRAGE AN FASTAPI (PORT 8000) & BACKGROUND.JS
  // ============================================================================
